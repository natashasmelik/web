import { startGame, changePlayer, endGame, setSendMessage, resultPut } from './game.js';
...
function processMessage( sendMessage: SendMessage, message: AnyServerMessage ): void
{
	switch ( message.type )
	{
		case 'gameStarted':
			startGame();
			changePlayer( message.myTurn, message.field, message.fieldOpposite, message.state );
			break;
		...
		case 'resultPut':
			resultPut( message.state, message.myTurn );
			break;
